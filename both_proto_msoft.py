...
def proto_input_train(n):
	n.data, n.label = L.ImageData(source='./data_trainf6.txt' ,batch_size=batch_s, ntop=2, root_folder = "/mnt/hdd/dataset/audioset/train_spectrogram_25ms_6frame/", include=[dict(phase=0)])
...
def proto_input_val(n):
	n.data, n.label = L.ImageData(source='./data_valf6.txt' ,batch_size=batch_s, ntop=2, root_folder = "/mnt/hdd/dataset/audioset/eval_spectrogram_25ms_6frame/", include=[dict(phase=1)])
	n.silence_label = L.Silence(n.label, ntop=0)
...
def proto_loss(n):
	...
	#n.loss = L.SoftmaxWithLoss(n.fc6, n.rabel)
	n.logp   = L.Log(n.score)
	n.ylogp = L.Eltwise(n.logp,n.rabel , operation=P.Eltwise.PROD)
	n.loss = L.Reduction(n.ylogp,axis=0, operation=P.Reduction.ASUM,loss_weight = 1)
	#n.finlos = L.Eltwise(n.suml, operation=P.Eltwise.PROD, coeff=-1.0,loss_weight=1)
...
with open(osp.join('./', 'net_train_msoft.prototxt'), 'w') as f:
	f.write(proto_train())
with open(osp.join('./', 'net_val_msoft.prototxt'), 'w') as f:
	f.write(proto_val())
