...
def proto_input_train(n):
	n.data, n.label = L.ImageData(source='./train4.txt' ,batch_size=batch_s, ntop=2, root_folder = "/mnt/hdd/dataset/audioset/train_spectrogram_25ms_6frame/", include=[dict(phase=0)])
	n.silence_label = L.Silence(n.label, ntop=0)
	
	n.rabel  = L.HDF5Data(source='./label_train.txt',batch_size=batch_s,include=[dict(phase=0)])
	#slicing label
...
def proto_input_val(n):
	n.data, n.label = L.ImageData(source='./val4.txt' ,batch_size=batch_s, ntop=2, root_folder = "/mnt/hdd/dataset/audioset/train_spectrogram_25ms_6frame/", include=[dict(phase=1)])
	n.silence_label = L.Silence(n.label, ntop=0)
	
	n.rabel  = L.HDF5Data(source='./label_val.txt',batch_size=batch_s,include=[dict(phase=1)])
	#slicing label
	tmp_tops =  L.Slice(n.rabel, slice_param={'axis':1}, ntop=cl)
	for idx in xrange(cl):
		n.tops['lr%03d'%idx] = tmp_tops[idx]
...
with open(osp.join('./', 'net_train.prototxt'), 'w') as f:
	f.write(proto_train())
with open(osp.join('./', 'net_val.prototxt'), 'w') as f:
	f.write(proto_val())
