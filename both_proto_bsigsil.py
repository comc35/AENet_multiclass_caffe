...
def proto_model(n):
	...
	n.silsig = L.Silence(n.sig,ntop=0)
	#slicing feature
...
def proto_loss(n):
	for i in xrange(cl):
		...
		n.tops['sigl%03d'%i] = L.SigmoidCrossEntropyLoss(n.tops['ft%03d'%i], n.tops['lr%03d'%i], loss_weight=0.002)#1/527 = 0.0019
	...
	#n.finsil = L.Silence(*[n.tops[item] for item in loss_layers], ntop=0)
...
with open(osp.join('./', 'net_train_bsigsil.prototxt'), 'w') as f:
	f.write(proto_train())
with open(osp.join('./', 'net_val_bsigsil.prototxt'), 'w') as f:
	f.write(proto_val())
