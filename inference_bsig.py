...
model_def = './deploy_sval.prototxt'
model_weights = './training/bsig_iter_100000.caffemodel'
...
with open('./data_valf6_short.txt') as f:
    content = f.readlines()
...
for row in bar(content):
	...
	net.forward()
	output_prob = net.blobs['prob'].data[0] #(527,)
	probs[i] = output_prob
	i +=1
